...
prompt = ChatPromptTemplate(
    messages=[
        SystemMessagePromptTemplate.from_template(
            "You are a helpful chatbot engaged in a conversation."
        ),
...
class AgentChat:
    def __init__(self, openai_api_key: str = settings.openai_api_key):
        ...
        self.agent = ConversationalChatAgent(llm_chain=llm_chain, prompt="You are an AI chatbot engaged in a conversation.", verbose=True)
        self.agent_executor = AgentExecutor.from_agent_and_tools(
            self.agent, tools=[], verbose=True
        )
    ...
    async def send_response(self, session_id: str, incoming_message: str):
        ...
        response_content = self.agent.aplan(
            intermediate_steps=session["message_history"], user_input=incoming_message
        )
        # Construct the response Message
        response = Message(content=response_content, sender="ai")
        # Save the response to the conversation buffer
        self.memory.save_context(
            {"input": incoming_message}, {"output": response_content}
        )
